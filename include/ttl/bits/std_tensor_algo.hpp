...
namespace internal
{
template <typename R, typename S, typename N = typename S::dimension_type>
N argmax(const basic_host_tensor_view<R, 1, S> &t)
{
    return std::max_element(t.data(), t.data_end()) - t.data();
}

template <typename R, typename R1, rank_t r, typename S>
void cast(const basic_host_tensor_view<R, r, S> &x,
          const basic_host_tensor_ref<R1, r, S> &y)
{
...
void fill(const basic_host_tensor_ref<R, r, S> &t, const R &x)
{
...
D hamming_distance(const basic_host_tensor_view<R, r, S> &x,
                   const basic_host_tensor_view<R, r, S> &y)
{
...
template <typename R, rank_t r, typename S>
R max(const basic_host_tensor_view<R, r, S> &t)
{
    return *std::max_element(t.data(), t.data_end());
}

template <typename R, rank_t r, typename S>
R min(const basic_host_tensor_view<R, r, S> &t)
{
    return *std::min_element(t.data(), t.data_end());
}

template <typename R, rank_t r, typename S>
R sum(const basic_host_tensor_view<R, r, S> &t)
{
    return std::accumulate(t.data(), t.data_end(), static_cast<R>(0));
}

template <typename R, rank_t r, typename S>
R mean(const basic_host_tensor_view<R, r, S> &t)
{
